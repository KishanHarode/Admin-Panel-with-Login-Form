...
const Update = () => {
    ...
    try {
      fetch(`${apiUrl}/${id}`)
        ...
        .then((json) => {
          ...
          setBloodType(json.BloodType);
          setDiagnosis(json.Diagnosis);
          setDate(json.date);
          setTreatment(json.Treatment);
          console.log("Data Fetch Successfully")
        });
    } catch (error) {
      console.warn("Fetch Update Data Error", error)
    }
  };
  ...
  const handleUpdate = (event) => {
    ...
    if (validation()) {
      const updateData = { PatientName, Age, Gender, BloodType, Diagnosis, date, Treatment };
      try {
        fetch(`${apiUrl}/${id}`, {
          method: "PUT",
          headers: {
        ...
        <div className="border border-danger" style={{ display: "flex", flexWrap: "wrap", padding: "10px", width: "100%" }}>
          <div className='container-fluid p-1' style={{ padding: "0px", fontFamily: "Arial, sans-serif", borderTop: "5px solid #28a745" }}>
            <div className='heading'>
              <h5 className='card-title fontheading1' style={{ fontFamily: 'sans-serif', padding: "5px" }}> Update Registration</h5>
            </div>
            <div className='mb-3'>
